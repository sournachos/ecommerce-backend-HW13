...
router.get('/:id', async (req, res) => {
  ...
  try{
    ...
    if(!productData) {res.status(404).json({ message: 'Could Not Find Product Data With That ID.'});return}
  ...
  Product.create(req.body)
    .then((product) => {
      ...
      if (req.body.tagIds.length) {
        const productTagIdArr = req.body.tagIds.map((tag_id) => {return {product_id: product.id,tag_id}});
...
router.delete('/:id', async (req, res) => {
  ...
  try{
    ...
    if (!productData) {res.status(404).json({ message: 'Could Not Find The Product Information' });return}
